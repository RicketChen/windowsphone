...
//	sprintf(szDCBparam , "baud=%d parity=%c data=%d stop=%d", baud, parity, databits, stopsbits);
	szDCBparam.Format(("baud=%d parity=%c data=%d stop=%d"),baud,parity,databits,stopsbits);
			...
			//	std::cout << cRecved;
				if(NTALK2)
					if(mp3fp > 0)
						...
						LOG(WARNING) << "record fail!";
				line[linelen++] = cRecved;
				if (cRecved == '\n')
				{
					line[linelen] = 0;
					linelen = 0;
					if (strstr(line, "+NTALK: 0"))
					{
						if (mp3fp > 0)
						{
							fclose(mp3fp);
					...
					if (NTALK2 || NTALK1)
					{
						...
						{
							strcat(receive, line);
						//	cout << soft_GetNowTime("%Y-%m-%d-%H:%M:%S").c_str() << ": " << line;
						}
